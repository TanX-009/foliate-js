...
class Reader {
  ...
  constructor() {
    ...
    const menu = createMenu([
      {
        ...
        onclick: (value) => {
          ...
          localStorage.setItem("foliate-layout", value);
        },
      },
    ]);
    ...
    const savedLayout = localStorage.getItem("foliate-layout");
    if (savedLayout) {
      menu.groups.layout.select(savedLayout);
    }
  }
  async open(file) {
    ...
    const initialLayout = localStorage.getItem("foliate-layout") || "paginated";
    this.view?.renderer.setAttribute("flow", initialLayout);
  ...
  async resumeProgress(progress) {
    if (progress !== null && progress !== undefined) {
      await this.view.init({ lastLocation: { fraction: progress } });
    } else {
  ...
  #onRelocate({ detail }) {
    ...
    // ✅ Save progress using globalThis.reader.fileName
    if (this.fileName && location) {
      saveProgress(this.fileName, fraction);
    }
  }
}

const open = async (file, fileName, progress) => {
  const reader = new Reader();
  globalThis.reader = reader;
  reader.fileName = fileName;
  await reader.open(file);
  await reader.resumeProgress(progress);
};
...
const loadFromURLParam = async () => {
  const params = new URLSearchParams(window.location.search);
  const fileUrl = params.get("file");

  if (!fileUrl) {
    console.error("No file URL specified in search params.");
    return;
  }
  ...
  try {
    ...
    // ✅ After loading file, apply saved progress
    const progress = await loadProgress(fileName);
    await open(file, fileName, progress);

    // if (saved) {
    //   await globalThis.reader.view.goTo(saved);
    // }
  } catch (err) {
